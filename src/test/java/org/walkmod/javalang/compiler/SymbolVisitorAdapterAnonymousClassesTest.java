...
public class SymbolVisitorAdapterAnonymousClassesTest extends SymbolVisitorAdapterTestSupport {
    ...
    public void testConditionalCompilationEliminatesAnonymousClasses() throws Exception {
        ...
        final TypeDeclarationAssert clazz = AstAssertions.assertThat(cu).types().item(0).hasName("U");
        clazz.members().item(5).asMethodDeclaration().hasName("resolvedBefore").body().stmts().item(0)
                .asExpressionStmt().expression().asMethodCallExpr().hasName("set").args().item(0).symbolData().name()
                .isEqualTo("U$1");

        // 14 (disabled) anonymous classes in between
        clazz.members().item(7).asMethodDeclaration().hasName("resolvedAfter").body().stmts().item(0).asExpressionStmt()
                .expression().asMethodCallExpr().hasName("set").args().item(0).symbolData().name().isEqualTo("U$16");
    }
    ...
    public void testConditionalCompilationFallsBackToBaseType() throws Exception {
        ...
        CompilationUnit cu = run(code);
        final TypeDeclarationAssert clazz = AstAssertions.assertThat(cu).types().item(0).hasName("U");
        clazz.members().item(2).asMethodDeclaration().hasName("resolvedBefore").body().stmts().item(0)
                .asExpressionStmt().expression().asMethodCallExpr().hasName("set").args().item(0).symbolData().name()
                .isEqualTo("U$1");
        clazz.members().item(4).asMethodDeclaration().hasName("resolvedAfter").body().stmts().item(0).asExpressionStmt()
                .expression().asMethodCallExpr().hasName("set").args().item(0).symbolData().name().isEqualTo("U$3");

        // no class could be loaded but the symbol type should at least resolve to
        // the base class
        clazz.members().item(3).asMethodDeclaration().hasName("deadCode").body().stmts().item(0).asIfStmt().thenStmt()
                .asBlockStmt().stmts().item(0).asExpressionStmt().expression().asMethodCallExpr().hasName("set").args()
                .item(0).symbolData().name().isEqualTo("java.io.Serializable");
    }
